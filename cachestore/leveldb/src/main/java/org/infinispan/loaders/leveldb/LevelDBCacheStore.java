...
public class LevelDBCacheStore extends LockSupportCacheStore<Integer> {
	...
	public void start() throws CacheLoaderException {
		...
		try {
			...
			expiryDb = openDatabase(config.getExpiraryLocation(),
					config.getExpiryDbOptions());
		} catch (IOException e) {
	...
	protected void reinitAllDatabases() throws IOException {
		...
		expiryDb = reinitDatabase(config.getExpiraryLocation(),
				config.getExpiryDbOptions());
	}
	...
	protected void purgeInternal() throws CacheLoaderException {
		try {
			...
			for (ExpiryEntry entry : entries) {
				...
				final byte[] existingBytes = expiryDb.get(expiryBytes);
				...
				if (existingBytes != null) {
					...
					if (existing instanceof List) {
						...
						expiryDb.put(expiryBytes, marshall(existing));
					} else {
						List<Object> al = new ArrayList<Object>(2);
						al.add(existing);
						al.add(entry.key);
						expiryDb.put(expiryBytes, marshall(al));
					}
				} else {
					expiryDb.put(marshall(expiryBytes), keyBytes);
				}
			}

			List<Long> times = new ArrayList<Long>();
			List<Object> keys = new ArrayList<Object>();
			DBIterator it = expiryDb.iterator(new ReadOptions()
					.fillCache(false));
			try {
				for (it.seekToFirst(); it.hasNext();) {
					Map.Entry<byte[], byte[]> entry = it.next();
